...
pub async fn list_tickets_by_user(
    pool: &MySqlPool,
    user_id: Uuid,
) -> Result<Vec<JiraTicket>, sqlx::Error> {
...
pub async fn find_ticket_by_id(
    pool: &MySqlPool,
    id: Uuid,
) -> Result<Option<JiraTicket>, sqlx::Error> {
    sqlx::query_as::<_, JiraTicket>("SELECT * FROM jira_tickets WHERE id = ?")
        .bind(id)
        .fetch_optional(pool)
        .await
}

pub async fn find_ticket_by_id_and_user(
    pool: &MySqlPool,
    id: Uuid,
    user_id: Uuid,
) -> Result<Option<JiraTicket>, sqlx::Error> {
    sqlx::query_as::<_, JiraTicket>("SELECT * FROM jira_tickets WHERE id = ? AND user_id = ?")
        .bind(id)
        .bind(user_id)
        .fetch_optional(pool)
        .await
...
pub async fn update_ticket_summary(
    pool: &MySqlPool,
    id: Uuid,
    summary: &str,
) -> Result<u64, sqlx::Error> {
    let result = sqlx::query("UPDATE jira_tickets SET ticket_summary = ? WHERE id = ?")
        .bind(summary)
        .bind(id)
        .execute(pool)
        .await?;
...
pub async fn delete_ticket(pool: &MySqlPool, id: Uuid, user_id: Uuid) -> Result<u64, sqlx::Error> {
    let result = sqlx::query("DELETE FROM jira_tickets WHERE id = ? AND user_id = ?")
        .bind(id)
        .bind(user_id)
        .execute(pool)
        .await?;
...
pub async fn update_tickets_order(
    ...
    for (index, id) in ticket_ids.iter().enumerate() {
        sqlx::query("UPDATE jira_tickets SET sort_order = ? WHERE id = ? AND user_id = ?")
            .bind(index as i32)
            .bind(id)
            .bind(user_id)
            .execute(&mut *tx)
